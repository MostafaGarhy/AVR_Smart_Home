...
u8 KPD_U8GetPressedKey(void)
{
	...
	for(row = 0; row < 4; row++)
	{
		...
		DIO_VidSetPinValue(KPD_PORT, row, PIN_U8_VALUE_LOW);
		for(col = 4; col < 8; col++)
		{
			//Check the Col_Input Pin if it is Low
			DIO_VidGetPinValue(KPD_PORT, col, &Pin_value);
			if(Pin_value == PIN_U8_VALUE_LOW)
			{
				_delay_ms(20); //Debouncing
				DIO_VidGetPinValue(KPD_PORT, col, &Pin_value);
				while(Pin_value == PIN_U8_VALUE_LOW)
				{
					DIO_VidGetPinValue(KPD_PORT, col, &Pin_value);
				}
		...
		DIO_VidSetPinValue(KPD_PORT, row, PIN_U8_VALUE_HIGH);
		if(Local_u8Flag == 1)
		{
			break;
		}
